...
def tileencode_cli():
    ...
    encoded_value = tileencode(args.number)
...
def tiledecode_cli():
    ...
    try:
        decoded_value = tiledecode(args.encoded)
    ...
    if bounds:
        ...
        feature = {
            ...
            "properties": {
                ...
                "resolution": z  # Using the code length as resolution
            }
        }
...
def children2geojson(tilecode):
    """
    Save the four children of a given tilecode to separate GeoJSON files.

    Args:
        tilecode (str): The parent tilecode.
    """
    # Get the child tilecodes
    children = tilecode_children(tilecode)

    # Save each child as a GeoJSON file
    for i, child_tilecode in enumerate(children):
        feature_collection = tilecode2geojson(child_tilecode)
        
        # Create a filename for each child based on its tilecode
        filename = f"{child_tilecode}.geojson"
        
        # Save the GeoJSON feature to a file
        with open(filename, 'w') as file:
            json.dump(feature_collection, file, indent=2)
        # print(f"Saved {child_tilecode} to {filename}")
...
def neighbors2geojson(tilecode):
    """
    Save the neighbors of a given tilecode to separate GeoJSON files.

    Args:
        tilecode (str): The parent tilecode.
    """
    # Get the neighbor tilecodes
    neighbors = tilecode_neighbors(tilecode)

    # Save each neighbor as a GeoJSON file
    for neighbor_tilecode in neighbors:
        feature_collection = tilecode2geojson(neighbor_tilecode)
        
        # Create a filename for each neighbor based on its tilecode
        filename = f"{neighbor_tilecode}.geojson"
        
        # Save the GeoJSON feature to a file
        with open(filename, 'w') as file:
            json.dump(feature_collection, file, indent=2)
        # print(f"Saved {neighbor_tilecode} to {filename}")
...
# print(feature_tilecodes(polygon, zoom_level))
...
# print(bbox_tilecodes(bbox,8))

# # lat,long = 10.48781200, 106.17187500
# # tilecode = latlong2tilecode(lat,long,8)
# tilecode= 'z9x407y240'
# print(tilecode)
# print(len(tilecode))
# quadkey = tilecode2quadkey(tilecode)
# print(quadkey)
# print(len(quadkey))
# print(tilecode2latlon(tilecode))
# area = tilecode_area(tilecode)
# print("Area of", tilecode, "is:", area, "square meters")
# # tilecode = 'z8x11y14'
# edge_length = tilecode_edge_length(tilecode)
# print(edge_length)
# tilebound = tilecode2tilebound(tilecode)
# print("Tile bound:", tilebound)

# bound = tilecode2bound(tilecode)
# print("Bound:", bound)


# wkt = tilecode2wktbound(tilecode)
# print("tilecode:", tilecode)
# print("Bounding box (WKT):", wkt)
# zoom = 5  # Specify the zoom level
# tilecodes = tilecode_list(zoom)
# print(len(tilecodes))
# quadkey = tilecode2quadkey(tilecode)
# print("quadkey:", quadkey)

# tilecode = quadkey2tilecode(quadkey)
# print("tilecode:", tilecode)