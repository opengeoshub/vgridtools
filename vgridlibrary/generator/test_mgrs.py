import mgrs
def get_unique_mgrs_codes(resolution):
    # Define MGRS converter

    # Set up bounding box for the entire world
    lat_min, lat_max = -79.0, 83.0  # MGRS is valid between these latitudes
    lon_min, lon_max = -180.0, 180.0

    # Determine step size based on resolution
    if resolution == 0:
        step = 1    # 100 km grid cell
    if resolution == 1:
        step = 0.1    # 10 km grid cell
    elif resolution == 2:
        step = 0.01   # 1 km grid cell
    elif resolution == 3:
        step = 0.001  # 100 m grid cell
    elif resolution == 4:
        step = 0.0001 # 10 m grid cell
    elif resolution == 5:
        step = 0.00001 # 1 m grid cell
    else:
        step = 1 
    
    # Set to store unique MGRS codes
    unique_mgrs_codes = set()
    
    # Iterate over latitude and longitude
    lat = lat_min
    while lat <= lat_max:
        lon = lon_min
        while lon <= lon_max:
            # Convert lat/lon to MGRS code at the desired resolution
            mgrs_code = mgrs.toMgrs(lat, lon, resolution)
            unique_mgrs_codes.add(mgrs_code)
            lon += step
        lat += step
    
    return unique_mgrs_codes

# Example usage
resolution_level = 0 # Change this to desired resolution (0 to 5)
unique_codes = get_unique_mgrs_codes(resolution_level)

# Output result
print(f"Number of unique MGRS codes at resolution {resolution_level}: {len(unique_codes)}")
print("Sample MGRS codes:", list(unique_codes)[:10])  # Display a sample
# lat,lon = -80, 40
# print (mgrs.toMgrs(lat, lon, 0))