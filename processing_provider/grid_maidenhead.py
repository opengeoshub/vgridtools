...
class GridMaidenhead(QgsProcessingAlgorithm):
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        ...
        if self.precision < 1 or self.precision> 4:
            feedback.reportError('Precision parameter must be in range [1,4]')
            return False
    ...
    def maidenheadgrid(self, precision, feedback):
        grid_params = {
            1: (18, 18, 20, 10),
            2: (180, 180, 2, 1),
            3: (1800, 1800, 0.2, 0.1),
        ...
        if precision not in grid_params:
            raise ValueError("Unsupported precision")

        x_cells, y_cells, lon_width, lat_width = grid_params[precision]
    ...
    def maidenheadgrid_with_extent(self, precision, extent, feedback):
        # Define grid parameters based on precision level
        grid_params = {
            1: (18, 18, 20, 10),
            2: (180, 180, 2, 1),
            3: (1800, 1800, 0.2, 0.1),
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if self.grid_extent is None or self.grid_extent.isEmpty():
            grid_cells = self.maidenheadgrid(self.precision,feedback)
        else:
            grid_cells = self.maidenheadgrid_with_extent(self.precision, self.grid_extent, feedback)
