...
class BinOLC(QgsProcessingAlgorithm):
    INPUT = 'INPUT'
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterNumber(
            self.RESOLUTION,
            self.tr('Resolution [2, 4, 6, 8, 10, 11..15]'),
    ...
    def processAlgorithm(self, parameters, context, feedback):        
        ...
        for i, point_feature in enumerate(self.point_layer.getFeatures()):
            ...
            append_stats_value(olc_bins, olc_id, props_dict, self.stats, self.numeric_field, self.category_field)
        ...
        for cat, values in olc_bins[next(iter(olc_bins))].items():
        ...
        for i, (olc_id, geom) in enumerate(olc_geometries.items()):
            ...
            olc_feature.setAttributes([props.get(f.name(), None) if f.name() != 'olc' else olc_id for f in out_fields])
