...
class BinGeohash(QgsProcessingAlgorithm):
    INPUT = 'INPUT'
    ...
    def processAlgorithm(self, parameters, context, feedback):        
        ...
        for i, point_feature in enumerate(self.point_layer.getFeatures()):
            ...
            append_stats_value(geohash_bins, geohash_id, props_dict, self.stats, self.numeric_field, self.category_field)
        ...
        for cat, values in geohash_bins[next(iter(geohash_bins))].items():
        ...
        for i, (geohash_id, geom) in enumerate(geohash_geometries.items()):
            ...
            geohash_feature.setAttributes([props.get(f.name(), None) if f.name() != 'geohash' else geohash_id for f in out_fields])
