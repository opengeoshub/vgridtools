...
class GARSGrid(QgsProcessingAlgorithm):
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        ...
        if self.resolution > 1 and (
            ...
            feedback.reportError(
                "For performance reason, when resolution is greater than 1, the grid extent must be set."
            )
            return False
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if self.grid_extent is None or self.grid_extent.isEmpty():
            ...
            min_x = max(
                0, int((self.grid_extent.xMinimum() - lon_min) / resolution_degrees)
            )
            max_x = min(
                len(longitudes),
                int((self.grid_extent.xMaximum() - lon_min) / resolution_degrees) + 1,
            )
            min_y = max(
                0, int((self.grid_extent.yMinimum() - lat_min) / resolution_degrees)
            )
            max_y = min(
                len(latitudes),
                int((self.grid_extent.yMaximum() - lat_min) / resolution_degrees) + 1,
            )
