...
import os
import numpy as np
from ..vgridlibrary.geocode.gars import GARSGrid
...
class GridGARS(QgsProcessingAlgorithm):
    ...
    def tags(self):
        return self.tr('GARS, grid, generator').split(',')
    ...
    def prepareAlgorithm(self, parameters, context, feedback):
        precision_index = self.parameterAsEnum(parameters, self.PRECISION, context)
        precision_values = [30,15,5,1]
        self.precision = precision_values[precision_index]
        if self.precision not in  [30,15,5,1]:
            feedback.reportError('Precision parameter must be in [30,15,5,1] minutes')
            return False
    ...
    def garsgrid(self, precision, feedback):
    ...
    def garsgrid_with_extent(self, precision, extent, feedback):
        # Define bounds for the whole planet
        lon_min, lon_max = -180.0, 180.0
        lat_min, lat_max = -90.0, 90.0
